...
import json
import os
import buildgfx, downloadpicons
import E2Globals
...
class E2Piconizer_SelectPicons(Screen):
    ...
    def __init__(self, session):
        ...
        self.timer1 = eTimer()
        self.timer1.timeout.get().append(self.delayedLoad)
        self.timer1.start(1, True)
    ...
    def getLastModifiedDate(self, picon_list, i):
        ...
        if url != '':
            ...
            if response != '':
                ...
                self.picon_list[i]['sid'] = self.picon_list[i]['sid'].encode('utf-8')
                self.picon_list[i]['title'] = self.picon_list[i]['title'].encode('utf-8')
    ...
    def addHeader(self):
        ...
        print json.dumps(self.picon_list)
        templist = [x for x in self.picon_list if not x['status'] == False]
